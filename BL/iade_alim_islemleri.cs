...
namespace BL
{
    public class iade_alim_islemleri
    {
        ...
        public List<almaverme> kayit_listeleme1(int tc)
        {
            //listelemek istediğimiz verileri veritabanından çekiyoruz.
            OleDbDataReader dr = dl.DRVeriCek("Select * from almaverme where ogrenciid in (select ogrenciid from ogrenciler where tc ='" + tc + "') ", CommandType.Text);
            if (dr.HasRows)
            {
        ...
        {
            //listelemek istediğimiz verileri veritabanından çekiyoruz.
            OleDbDataReader dr = dl3.DRVeriCek("Select * from almaverme where ogrenciid in (Select ogrenciid from ogrenciler where tc='" + deger + "')", CommandType.Text);
            if (dr.HasRows)
            {
                //veritabanından çektiğimiz verileri liste olarak geri gönderiyoruz.
                List<almaverme> islem_kayit = new List<almaverme>();
